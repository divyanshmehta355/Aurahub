          ...
          <div className="hidden md:flex items-center space-x-4">
            <div className="relative w-72 lg:w-96">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center">
                {searchTerm && (
                  <button onClick={handleClearSearch} className="px-3 text-gray-400 hover:text-gray-600">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                )}
                <button onClick={handleSearchSubmit} className="px-3 text-gray-400 hover:text-blue-600">
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="relative" ref={profileRef}>
                  <button onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)} className="flex items-center gap-2 text-gray-600 hover:text-blue-600">
                    {user?.image ? (
                        <Image 
                            src={user.image} 
                  ...
                  {isProfileMenuOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-xl border overflow-hidden">
                      <ul>
                        ...
                        <li><Link href="/dashboard" onClick={() => setIsProfileMenuOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Dashboard</Link></li>
                        <li><Link href="/my-playlists" onClick={() => setIsProfileMenuOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">My Playlists</Link></li>
                        <li><Link href="/subscriptions" onClick={() => setIsProfileMenuOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Subscriptions</Link></li>
            ...
            <div className="flex flex-col items-start space-y-2">
              {isAuthenticated ? (
                <>
                  ...
                  <Link href="/my-playlists" onClick={() => setIsMenuOpen(false)} className="w-full text-left py-2 px-2 text-gray-800 font-semibold rounded-md hover:bg-gray-100">My Playlists</Link>
                  ...
                  <button onClick={handleLogout} className="w-full text-left py-2 px-2 text-red-600 font-semibold rounded-md hover:bg-gray-100">Logout</button>
                </>
              ) : (
                <>
